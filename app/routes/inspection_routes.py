...
def get_by_id(id: int, db:Session(get_db), user = Depends(get_current_user)):
    return inspection_controller.get_inspection(id, db)


@router.put("/inspections/{id}")
def update(id: int, data: InspectionUpdate, db: Session(get_db), user = Depends(get_current_user)):
    return inspection_controller.update_inspection(id, data, db)

@router.delete("/inspections/{id}")
def delete(id: int, db: Session(get_db), user = Depends(get_current_user)):
