...
def get_booking(id: int, db: Session = Depends(get_db), user = Depends(get_current_user)):
    return booking_controller.get_booking(id, db)

@router.put("/bookings/{id}")
def update_booking(id: int, data: BookingUpdate, db: Session = Depends(get_db), user = Depends(get_current_user)):
    return booking_controller.update_booking(id, data, db)

@router.delete("/bookings/{id}")
def delete_booking(id: int, db: Session = Depends(get_db), user = Depends(get_current_user)):
